...
export default function NotificationsPage() {
  ...
  return (
    <div className="container-content max-w-2xl py-6">
      ...
      {items.length === 0 ? (
        <div className="card p-10 text-center text-sm text-black/60 dark:text-white/65">{t("notif.empty")}</div>
      ) : (
            ...
            const inner = (
              <div className={`card flex items-start gap-3 p-4 ${!n.read ? "border-l-4 border-l-accent" : ""}`}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs text-black/60 dark:text-white/65">{new Date(n.createdAt).toLocaleString(loc)}</p>
                </div>
