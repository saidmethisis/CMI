...
const statusKey: Record<string, string> = { active: "ui.stActive", blocked: "ui.stBlocked", disabled: "ui.stDisabled" };
...
export default function StaffPage() {
  ...
  const create = async () => {
    ...
    const j = await r.json(); if (!r.ok) { setMsg(j.error?.message || t("misc.error")); setTimeout(() => setMsg(""), 3000); return; }
    setCreating(false); setForm({ name: "", email: "", roleSlug: "writer", password: "" }); load();
  };
  const loginAs = async (u: User) => {
    const r = await fetch("/api/admin/login-as", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ userId: u.id }) });
    const j = await r.json();
    if (!r.ok) { setMsg(j.error?.message || t("misc.error")); setTimeout(() => setMsg(""), 3000); return; }
    // теперь мы — этот пользователь; ведём в его рабочее пространство
    window.location.href = u.roleSlug === "reader" ? "/" : "/author-panel";
  };
    ...
    <div>
      ...
      <div className="card overflow-x-auto">
        <table className="w-full min-w-[640px] text-sm">
          <thead><tr className="text-left text-xs uppercase text-black/60 dark:text-white/65"><th className="px-4 py-2 font-medium">{t("a.hUsers")}</th><th className="px-4 py-2 font-medium">{t("a.role")}</th><th className="px-4 py-2 font-medium">{t("a.status")}</th><th className="px-4 py-2 font-medium">{t("a.actions")}</th></tr></thead>
          <tbody>
            {users.map((u) => (
              <tr key={u.id} className="border-t border-black/5 dark:border-white/10">
                <td className="px-4 py-3"><div className="font-medium">{u.name}</div><div className="text-xs text-black/60">{u.email}</div></td>
                ...
                <td className="px-4 py-3"><span className={`chip !py-0.5 text-[11px] ${statusMap[u.status] ?? ""}`}>{statusKey[u.status] ? t(statusKey[u.status]) : u.status}</span></td>
                <td className="px-4 py-3">
