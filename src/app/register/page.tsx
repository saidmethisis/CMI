    ...
    <div className="container-content grid min-h-[70vh] place-items-center py-10">
      <div className="card w-full max-w-sm p-6">
        ...
        <div className="mt-5 space-y-3">
          <div><label className="label" htmlFor="reg-name">{t("auth.name")}</label><input id="reg-name" autoComplete="name" className="ym-disable-keys input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div>
          <div><label className="label" htmlFor="reg-email">{t("auth.email")}</label><input id="reg-email" autoComplete="email" className="ym-disable-keys input" type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="you@mail.com" /></div>
          <div><label className="label" htmlFor="reg-password">{t("auth.password")}</label><input id="reg-password" autoComplete="new-password" className="ym-disable-keys input" type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder={t("au.passwordMin")} /></div>
