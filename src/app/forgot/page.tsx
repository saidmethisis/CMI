    ...
    <div className="container-content grid min-h-[70vh] place-items-center py-10">
      <div className="card w-full max-w-sm p-6">
        ...
        <div className="mt-4 space-y-3">
          <div><label className="label" htmlFor="fg-email">{t("auth.email")}</label><input id="fg-email" autoComplete="email" className="ym-disable-keys input" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@mail.com" /></div>
          <button className="btn-ghost w-full" onClick={request}>{t("au.getToken")}</button>
          {issued && (
            <>
              <div><label className="label" htmlFor="fg-token">{t("au.resetToken")}</label><input id="fg-token" autoComplete="one-time-code" className="ym-disable-keys input" value={token} onChange={(e) => setToken(e.target.value)} placeholder={issued} /></div>
              <div><label className="label" htmlFor="fg-password">{t("au.newPassword")}</label><input id="fg-password" autoComplete="new-password" className="ym-disable-keys input" type="password" value={password} onChange={(e) => setPassword(e.target.value)} /></div>
