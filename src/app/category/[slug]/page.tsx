...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  const c = (await getCategories()).find((x) => x.slug === slug);
  if (!c) notFound();
  let items = localizeList(await listPublished({ category: slug, q: sub || undefined }), lang);
  if (sort === "pop") items = [...items].sort((a, b) => b.views - a.views);
  const subs = subsectionPairs(lang, slug);
    ...
    <div className="container-content py-6">
      ...
      {subs.length > 0 && (
        <div className="no-scrollbar -mx-4 mb-4 flex gap-2 overflow-x-auto px-4 sm:mx-0 sm:px-0">
          ...
          {subs.map((s) => (
            <Link key={s.value} href={`/category/${slug}?sub=${encodeURIComponent(s.value)}`} className={`chip ${sub === s.value ? "bg-brand text-white" : "hover:bg-black/5 dark:hover:bg-white/10"}`}>{s.label}</Link>
          ))}
        </div>
      )}
