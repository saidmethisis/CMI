...
if [ "$HEALTHY" -ne 1 ]; then
  ...
  # Ищем именно ПРОВАЛ миграции, а не слово «migrate» в логе. Контейнер каждый
  # раз печатает вывод `prisma migrate deploy`, поэтому прежнее условие срабатывало
  # на любом неудачном деплое и отправляло восстанавливать базу из дампа там, где
  # с базой всё в порядке. Признаки настоящей беды: код ошибки Prisma либо то, что
  # Next вообще не дошёл до запуска.
  if echo "$LOGS" | grep -qiE "P[0-9]{4}|migrate found failed migrations|migration.*(failed|error)" \
     || ! echo "$LOGS" | grep -q "Ready in"; then
